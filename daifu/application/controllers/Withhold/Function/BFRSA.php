...
//		var_dump($private_key_password);die;
...
//		print_r($this->private_key);die;
		...
		while ($encryptPos < $totalLen)
		{
			openssl_private_encrypt(
				substr($data_content, $encryptPos, BAOFOO_ENCRYPT_LEN),
				$encryptData,
				$this->private_key
			);
	...
	function decryptByPublicKey($encrypted)
	{
		...
		{
			$res = openssl_public_decrypt(
				hex2bin(substr($encrypted, $decryptPos, BAOFOO_ENCRYPT_LEN * 8)),
				$decryptData,
				$this->public_key
			);
			$decrypt .= $decryptData;
