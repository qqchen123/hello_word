...
class Rep_backplan extends CI_Controller {
	 ...
	 * @param $pwd
	 */
	public function yx_login($account = 'YX15637822759', $pwd = 'yxt822759', $status='')
	...
	public function get_finish($yx_account,$status)
	{
		...
		$lend_title_url = QueryList::Query(
			$sub_str_end,
			[
				'link' => ['.myInvestment_box_RepaymentIn_one_a1', 'href'],
			]
		)->data;
		...
		$title = QueryList::Query(
			$sub_str_end,
			[
				'title' => ['.myInvestment_box_RepaymentIn_one_a1', 'text'],
			]
		)->data;
		foreach ($lend_title_url as $k => $v)
		{
			$res = $this->get_back_plan($v['link'], $title[$k]['title'], $yx_account,$status);
			if ($res)
			{
				continue;
			}
	...
	public function get_back_plan($url, $title, $yx_account,$status)
	{
		$this->client = new GuzzleHttp\Client();
		$this->jar = unserialize($_SESSION['jar_15618988191']);
		...
		if (empty($arr_chunk))
		{
			//判断这个标题下是否有数据--如果有就继续执行，没有返回false跳出本次循环
	...
	{
		$ures = $this->db->select('id')->where('account', $account)->where('check_status', 1)->get(
			'fms_yx_account'
		)->row_array();
		$usres = $this->db
