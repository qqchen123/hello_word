 ...
 * @desc 银信总账号出借
 */
class YxOutMoney_model extends Admin_Model
{
	public $table_name = 'fms_yx_out_money';
	...
	public function list_out_money($like='',$page=1,$rows=10,$sort='reg_time',$order='DESC'){

		if ($like){
			$this->db->or_like('fms_yx_out_money.account',$like);
		...
		$sub_detail_sql = '(select `account`,`user_name`,`id_number`,`call_number` from `fms_yx_out_money_detail` GROUP BY `user_name`,`id_number`,`call_number`) as tmp_detail';

		//$sort = 'fms_yx_out_money.'.$sort;
		$this->db
			->join('fms_yx_account','fms_yx_account.account=fms_yx_out_money.account','left')
			->join('fms_user','fms_user.yx_account=fms_yx_out_money.account','left')
			->join($sub_detail_sql,'tmp_detail.account=fms_yx_out_money.account','left');
        ...
        $total = $this->db->count_all_results('fms_yx_out_money',false);
	...
	public function get_total(){
		$ret = $this->db->select('count(account) as total')
			->get($this->table_name)
			->row_array();
		return $ret['total'];
	}
