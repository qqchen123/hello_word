<?php
class Jigou_model extends CI_Model{
    ...
    function list_jg($like='',$page=1,$rows=10,$sort='jg_id',$order='ASC',$jg_id=null){

        $this->load->helper('publicstatus');
        joinStatus('jg','jg_id');

        if ($jg_id) $this->db->where('jg_id',$jg_id);
        if ($like) $this->db->like('jg_name',$like);
        $total = $this->db->count_all_results('fms_jigou_info',false);
