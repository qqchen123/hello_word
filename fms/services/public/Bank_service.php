...
class Bank_service extends Admin_service
{
	...
	/**
	 * @name 从魔蝎获取网银报告 落地到mongo
	 * @url https://api.51datakey.com/bank/v3/report?task_id={task_id}
	 * https://api.51datakey.com/bank/v3/allcards-ex2?task_id={task_id}  获取数据最全，推荐使用
	 * @param $mobile string
	 * @return boolean
	 */
	public function record_report($task_id)
	{
		$path = '/bank/v3/report?task_id=' . $task_id;
		$path1 = '/bank/v3/allcards-ex2?task_id=' . $task_id;
		$report = $this->moxie->exec_curl_get_data('', $path);
		$report1 = $this->moxie->exec_curl_get_data('', $path1);
		$report = [
			'report' => ArrayHelper::object_to_array($report),
			'allcards-ex2' => ArrayHelper::object_to_array($report1)
		];
		if (!empty($report)) {
