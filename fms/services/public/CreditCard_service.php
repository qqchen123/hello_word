 ...
 * @desc 网银信用卡serivce
 */
class CreditCard_service extends Admin_service
{
	/**
	 * @var table name 
	 */
	public $table_name = 'credit_card_report';
	...
	public function record_report($task_id)
	{
		...
		if (!empty($report)) {
			...
			if ($res) {
				Slog::log('网银信用卡报告落地成功');
				return true;
			} else {
				Slog::log('网银信用卡报告写入失败');
			}
		} else {
			Slog::log('获取网银信用卡报告失败');
		}
		return false;
	}
