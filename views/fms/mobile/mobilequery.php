...
<script type="text/javascript">
    ...
    var typemap = JSON.parse('<?= $typemap ?>');
...
<script type="text/javascript" src="/assets/apps/user/mobilequery.js?s=<?= time()?>"></script>
...
/*##########################################*/
    ...
    function change_color(color) {
        ...
        // var add_map = [
        //     'background-color:#f7de8f !important;color:white !important;',//黄
        //     'background-color:#de2531 !important;color:white !important;',//红
        //     'background-color:#8fd850 !important;color:white !important;'//绿
        // ];
        var add_map = [
        ...
        $.each(color, function(i, item){
            ...
            // tmp_style = $('#submitForm #'+i).next('span').children('input:eq(0)').attr('style');
            if (undefined != tmp_style) {
                ...
                for (var z = 0; z < color_map.length; z++) {
                    ...
                    if (flag) {
                        ...
                        // $('#submitForm #'+i).next('span').children('input:eq(0)').attr('style', tmp_style + add_map[z]);
...
//########################################################
    ...
    function add(fuserid, idnumber, name, channel) {
        //调用 数据处理 装载数据 打开新增窗口
        no_status();
        $('#submit-dlg').dialog('open').dialog('setTitle', '增加手机卡');
        $('#submitForm').form('clear');
        $('#submitForm #classBtn .l-btn-text').text('新增');
        $('#edit-sub').attr('data-url', 'add/mobile');
