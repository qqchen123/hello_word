...
<div class="easyui-layout" data-options="fit:true,border:false">
    <div region="north" data-options="border:false" style="padding: 8px 20px;">
        ...
        <div class="page-header">
            <span class="bigger-150">
                <a onclick="location.href='<?php echo site_url('Qudao/query')?>'">渠道列表</a>  /  <a onclick="location.href='<?php echo site_url('Qudao/getpickers').'/'.$qinfo['q_id']?>'">渠道【<?php echo $qinfo['q_name']?>】对接人信息表</a> / 修改对接人
            </span>
        </div><!-- /.page-header -->
        <form id='fileform' method='post' action='<?php echo site_url('Qudao/savedc_row')?>' target="uploadFrame">
            <table class="table table-bordered" style="margin: 0;padding: 0px">
                <tbody>
                <tr>
                    <td class="tlabel">机构名称</td>
                    <td>
                        <select name="dc_jgid" id="dc_jgid" class="easyui-combogrid col-sm-3" style="height: 30px;line-height: 30px" required _required _valid="isChineseORAlphar" _msg="机构名称错误" maxlength="50"></select>
                    </td>
...
<script>
    ...
    $(function () {
        ...
        var vals=<?php echo json_encode($dcinfo)?>;
        for(var idx in vals){
    ...
    function complete($code, $msg) {
        if($code==200){
            top.modalbox.alert('成功',function () {
                window.location.href='<?php echo site_url('Qudao/getpickers').'/'.$qinfo['q_id']?>';
            })
        }else{
