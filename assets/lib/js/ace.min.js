...
jQuery(function (a) {
    ...
    a('body').delegate('.noaction','click', function (e) {
        e.preventDefault();
    })
});
jQuery(function (a) {
...
ace.handle_side_menu = function (a) {
    ...
    a(".nav-list").on(ace.click_event, function (h) {
        ...
        var li= a(g).closest('li.active');

        if (!g || g.length == 0) {
            return
        }
        if(!a(g).siblings('ul.submenu').length){
            a(".nav-list li.active").removeClass('active');
            a(g).parent('li').addClass('active');
        }
        if(a(g).parents('ul.submenu').length){
            li = a(g).parents('li');
            a(li[li.length-1]).addClass('active');
        }
